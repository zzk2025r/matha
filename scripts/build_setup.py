...
用法:
    python scripts/build_setup.py              # 构建安装程序
    python scripts/build_setup.py --no-pyinstaller  # 仅生成 .ps1（无需 PyInstaller）
"""
...
def check_files() -> dict:
    """检查必要文件。"""
    return {
        "matha_exe": (DIST_DIR / "matha-offline" / "matha.exe").exists(),
        "matha_cc_exe": (DIST_DIR / "matha-cc-offline" / "matha-cc.exe").exists(),
        "pyinstaller": shutil.which("pyinstaller") is not None or (
            Path(__file__).parent.parent / ".venv" / "Scripts" / "pyinstaller.exe"
        ).exists(),
    }


def generate_setup_ps1(version: str) -> Path:
    """生成 PowerShell 安装脚本，然后打包为 exe。"""
    ps1_path = SCRIPTS_DIR / "setup_matha.ps1"

    content = f'''# Matha Windows 安装程序 - 双击运行
# 版本: {version}
...
'''
    ...
    log(f"生成安装脚本: {ps1_path}")
    return ps1_path


def build_setup_exe(version: str, use_pyinstaller: bool = True) -> Path | None:
    """使用 PyInstaller 构建安装程序 exe。"""
    if not use_pyinstaller:
        log("跳过 PyInstaller 打包，请手动双击 .ps1 运行安装")
        return None

    log("构建安装程序 exe...")
    ps1_path = generate_setup_ps1(version)

    # 包装脚本（创建 temp 目录并运行 ps1）
    wrapper_script = SCRIPTS_DIR / "_setup_wrapper.py"
    wrapper_script.write_text(f'''# -*- coding: utf-8 -*-
...
script_path = Path(__file__).parent / "_matha_setup.ps1"
...
''', encoding="utf-8")
    ...
    # 复制 ps1 到 build 目录
    build_ps1 = Path("build") / "_matha_setup.ps1"
    build_ps1.parent.mkdir(exist_ok=True)
    shutil.copy2(ps1_path, build_ps1)

    # 使用 PyInstaller 打包
    cmd = [
        sys.executable, "-m", "PyInstaller",
        "--noconfirm", "--clean",
        "--distpath", str(DIST_DIR / "setup"),
        "--workpath", str(BUILD_DIR / "setup"),
        "--name", "Matha-Setup",
        "--onefile",
        str(wrapper_script),
    ]
    ...
    if code != 0:
        log(f"  PyInstaller 打包失败，尝试直接打包 ps1...")
        # 尝试直接打包 ps1
        cmd2 = [
            sys.executable, "-m", "PyInstaller",
            "--noconfirm", "--clean",
            "--distpath", str(DIST_DIR / "setup"),
            "--workpath", str(BUILD_DIR / "setup2"),
            "--name", "Matha-Setup",
            "--onefile",
            str(ps1_path),
        ]
        log(f"  $ {' '.join(cmd2)}")
        code2, stdout2, stderr2 = run_cmd(cmd2)
        if code2 != 0:
            log(f"  两种方式都失败，将使用 .ps1 文件")
            return None
        exe_path = DIST_DIR / "setup" / "Matha-Setup.exe"
    else:
        exe_path = DIST_DIR / "setup" / "Matha-Setup.exe"

    if exe_path.exists():
        size = exe_path.stat().st_size
        log(f"  ✓ 安装程序: {exe_path.name} ({size // 1024 // 1024} MB)")
        return exe_path
    return None


def create_stub_exe(version: str) -> Path:
    """创建 stub exe（当 PyInstaller 不可用时，生成可执行的 .ps1 包装）。"""
    log("创建启动脚本...")
    ps1_path = generate_setup_ps1(version)

    # 创建一个 .bat 作为启动器
    bat_path = DIST_DIR / "Matha-Setup.bat"
    bat_content = f'''@echo off
REM Matha v{version} Windows 安装程序
echo ============================================================
...
'''
    ...
    log(f"  ✓ 安装启动器: {bat_path}")
    return bat_path


def main():
    parser = argparse.ArgumentParser(description="Matha Windows 安装程序构建工具")
    parser.add_argument("--version", default="4.4", help="版本号")
    parser.add_argument("--no-pyinstaller", action="store_true", help="不使用 PyInstaller")
    parser.add_argument("--force", action="store_true", help="强制重建")
    args = parser.parse_args()

    print("=" * 60)
    print("Matha Windows 安装程序构建工具")
    print("=" * 60)
    ...
    if not files["matha_exe"]:
        log("错误: 请先构建 matha.exe")
        log("  运行: python -m PyInstaller --noconfirm matha.spec")
        sys.exit(1)

    # 构建
    setup_dir = DIST_DIR / "setup"
    if args.force and setup_dir.exists():
        shutil.rmtree(setup_dir)
    setup_dir.mkdir(parents=True, exist_ok=True)

    exe_path = None
    if not args.no_pyinstaller:
        exe_path = build_setup_exe(args.version, use_pyinstaller=True)

    if not exe_path:
        log("使用 .bat 启动器作为备选方案")
        bat_path = create_stub_exe(args.version)
        print()
        print("=" * 60)
        print("安装程序构建完成！")
        print("=" * 60)
        print(f"启动安装: 双击 {bat_path}")
        print(f"或直接运行: powershell -ExecutionPolicy Bypass -File {SCRIPTS_DIR / 'setup_matha.ps1'}")
        print()
        print("快捷方式将在安装后自动创建:")
        print("  - 桌面: Matha REPL / Matha 编译器 / Matha 安装目录")
        print("  - 开始菜单: Matha 程序组")
        return
    ...
    print("安装程序构建完成！")
    print("=" * 60)
    print(f"安装程序: {exe_path}")
    print(f"大小: {exe_path.stat().st_size // 1024 // 1024} MB")
    print()
    print("使用方法:")
    print("  双击 Matha-Setup.exe 运行安装程序")
    print("  安装后自动创建桌面快捷方式")
    print()
