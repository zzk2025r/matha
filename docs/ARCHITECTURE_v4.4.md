...
│                              ▼                                               │
│              ┌──────────────┐  ┌──────────────┐  ┌──────────────┐          │
│              │Python Stdlib │  │ symbol_codegen│  │ multi_lang_  │          │
│              │(stdlib/*.py) │  │ (Python/C/JS) │  │ codegen.py   │          │
│              │mathlib.py    │  │              │  │              │          │
│              │physics_*     │  │              │  │              │          │
│              └──────────────┘  └──────────────┘  └──────────────┘          │
│                              │                                               │
...
│  3. Matha 在任何生态/系统/平台环境下独立安装使用                              │
│  4. 降级策略：tree-sitter 不可用时自动回退到正则解析                           │
│  5. 用户显式调用时按需加载（Lazy Loading）                                    │
│                                                                             │
│  ── 语言加载策略 ─────────────────────────────────────────────────────────── │
│  • Matha 核心    : 默认激活（lexer/parser/interp/REPL）                      │
│  • Python Stdlib: 非默认（stdlib/*.py, mathlib.py 按需 import）             │
│  • symbol_codegen: 非默认（Python/C/JS 代码生成按需调用）                    │
│  • Rust/Go/JS  : 非默认（tree-sitter 可选加载）                              │
│  • LLM 解析     : 非默认（anthropic/deepseek/openai 可选）                   │
└─────────────────────────────────────────────────────────────────────────────┘
```
